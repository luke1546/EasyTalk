...
const MyHomePage = () => {
  ...
  useEffect(() => {
    ...
    if (targetUserId) {
      axios.get(`/user?targetId=${targetUserId}`, { withCredentials: true })
        .then(response => {
          // const { userId, profileImg, userName, pageType } = response.data;   // 데이터 저장
          // console.log(response.data)
          // setuserData({ userId, profileImg, userName, pageType });
